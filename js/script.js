...
const gameData = {
    ...
    "workers": [
        {"code": "++++[->+<].", "held": 0, "active": false}
    ]
};

var generatorInterval = -1;
var workerExecutors = [
    {"executor": new bfworker(array, gameData.workers[0].code), "held": null, "active": null}
];
...
function displayWorkers() {
    ...
    for (let i = 0; i < gameData.workers.length; i++) {
        ...
        checkbox.checked = gameData.workers[i].active;
...
function workersUpdate() {
    for (let i = 0; i < workerExecutors.length; i++) {
        ...
        gameData.workers[i].active = workerExecutors[i].executor.active;
